...
def run_experiment(model='mamba_bilstm', dataset='Davis'):
    ...
    try:
        ...
        if os.path.exists(local_script):
            ...
            # Upload dataset_seq.py
            local_dataset_seq = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dataset_seq.py')
            remote_dataset_seq = 'MBG-DTI/dataset_seq.py'
            if os.path.exists(local_dataset_seq):
                print(f"Uploading {local_dataset_seq} to {remote_dataset_seq}...")
                sftp.put(local_dataset_seq, remote_dataset_seq)
